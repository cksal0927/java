...
<%--
	...
	일치하면 로그인 처리(세션), 불일치하면 아이디 또는 비밀번호가 틀렸다고 알림	
--%>
...
<%
	...
	if(!list.isEmpty()){  // 테이블 조회한 데이터를 arraylist에 저장한 경우
		for(Member m : list){
			if(m.getUserId().equals(id)){  // 입력한 아이디와 일치하는가?
				if(m.getUserPassword().equals(pw)){  // 입력한 비밀번호와 일치하는가?
					// 아이디와 비밀번호가 모두 일치하기 때문에 세션값 생성
					session.setAttribute("user", m);
					// m은 입력한 아이디와 비밀번호가 일치한 Member클래스 객체
