...
<%-- view.jsp --%>
<%-- id 파라미터의 값을 board 테이블의 board_id 컬럼으로 조회하여 board_id와 id 파라미터가 일치하는 게시글 데이터들을 가지고 와서 화면에 출력 --%>
...
<%
	...
	Board data;  // select sql문의 결과는 Board 클래스 객체 하나에 저장되는 데이터이기 때문에
	...
	try{
		...
		if(db.rs.next()){
			data = new Board(db.rs.getInt("board_id"), db.rs.getString("writer"), db.rs.getString("title"),
