		...
		<div id="boardListBox">
			...
			<%
				...
				try{
					...
					while(db.rs.next()){
						list.add(new Board(db.rs.getInt("board_id"), db.rs.getString("writer"), db.rs.getString("title"),
			...
			<tr>
				...
				<td class="title"><a href="?part=view&id=<%=row.getBoard_id()%>"><%=row.getTitle() %></a></td>
...
<script>
	// 글 작성을 클릭할 때 로그인 회원 중에 권한이 있는 사람만 작성 가능
	let lv = '<%=level%>';
	$("#wrBt").on("click", function(){  // 글 작성을 클릭하면
		if(lv == 3){
			location.href = "?part=boardWrite";  // 권한이 3이면 글 작성 이동
		}else{
			alert("글 작성 권한이 없습니다.");
		}
	});
