...
public class CommentDAO extends DBConnect{
	...
	public void save(int bid, String comment, String writer) {
		...
		String sql = "insert into comment(board_id, comment, writer, writer_date values(?,?,?,now())";
	...
	public List<CommentDTO> findByBoardId(int bid) {
		...
		try {
			...
			rs = pt.executeQuery();
			while(rs.next()) {
				list.add(CommentDTO.of(rs.getInt("comment_id"), rs.getInt("board_id"), rs.getString("writer"),
						rs.getString("comment"), rs.getDate("write_date").toLocalDate()));
			}
		}catch(SQLException e) {
